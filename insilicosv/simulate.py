...
from tests.test_utils import region_length
...
class SVSimulator:
    ...
    def __init__(self, config_path):
        ...
        # Reference where only DEL are removed
        self.regions_for_overlap = RegionSet()
        self.length_for_overlap = {}
        # INS regions that can be overlapped with their remaining length for overlap (updated by DEL overlaps)
        self.length_insertions = defaultdict(int)
        self.inserted_regions = RegionSet()
        self.inserted_lengths = {}
    ...
    def load_rois(self):
        ...
        for blacklist_region_file in utils.as_list(self.config.get('blacklist_regions', [])):
            ...
            if blacklist_region_file.lower().endswith('.bed'):
                ...
                chk(False,f'Cannot import blacklist regions from {blacklist_region_file}: '
    ...
    def construct_svs(self):
        ...
        logger.info(f'Constructed {len(self.svs)} SVs and {sum([len(svs) for svs, _, _ in self.recurrent_svs.values()])} recurrent SVs')
    ...
    def update_regions_for_overlap(self, sv):
        # Add available regions for insertion, remove deleted regions
        for operation in sv.operations:
            ...
            if operation.transform.transform_type == TransformType.DEL:
                ...
                start_breakend = operation.source_breakend_region.start_breakend
                op_length = if_not_none(origin_lengths[start_breakend], operation.source_region.length())
                op_start = operation.source_region.start
                # Check first if an insertion region has been overlapped
                if chrom in self.inserted_regions.chrom2itree:
                    insertion_overlap = list(self.inserted_regions.chrom2itree[chrom].overlap(
                        Interval(op_start, op_start + op_length)))
                    insertion_overlap = sorted(insertion_overlap, key=lambda x: x.begin)
                    for insertion in insertion_overlap:
                        # We chop the insertion by the overlapping DEL until the DEL has been all used.
                        if op_start + op_length < insertion.data.start: break
                        length_overlap = op_start + op_length - insertion.data.start
                        if length_overlap > self.inserted_lengths[insertion.data]:
                            op_length -= self.inserted_lengths[insertion.data]
                            self.length_insertions[chrom] -= self.inserted_lengths[insertion.data]
                            # The DEL is larger than the insertion, the inserted regions is removed
                            self.inserted_regions.chrom2itree[chrom].remove(insertion)
                            del self.inserted_lengths[insertion.data]
                        else:
                            # The inserted regions is bigger than the DEL, the lengths are updated
                            self.inserted_lengths[insertion.data] -= length_overlap
                            op_length -= length_overlap
                            self.length_insertions[chrom] -= length_overlap
                    # Adapt the region of the DEL overlapping the reference
                    operation.source_region = Region(start=op_start, end=op_start + op_length, chrom=chrom)
                # The part of the DEL that is not overlapping insertion targets is removed from the reference
                ...
                if operation.novel_insertion_seq:
                    ...
                    # Duplication
                    length = operation.source_region.length() * operation.transform.n_copies
                # A new region is inserted
                inserted_region = Region(start=operation.target_region.start,
                                                           end=operation.target_region.start,
                                                           chrom=operation.target_region.chrom,
                                                            op_id=operation.op_id)
                self.inserted_regions.add_region(inserted_region)
                self.inserted_lengths[inserted_region] = length
                self.length_for_overlap[operation.target_region.chrom] += length


    def call_place_sv(self, sv, sv_num, sv_set, roi_indexes):
        t_start_placing_sv = time.time()
        logger.debug(f'Placing {sv_num=} {sv=} and {sum([len(self.svs) for svs, _, _ in self.recurrent_svs.values()])} recurrent SVs')
        ...
        if self.overlap_sv:
            ...
            for sv_set, (svs, recurrence, recurrence_num) in self.recurrent_svs.items():
                ...
                if to_place:
                    ...
                    # Place the requested number of SVs per burst if enough SVs remain
    ...
    def place_svs(self):
        ...
        logger.info(f'Placing {len(self.svs)} svs and {sum([len(svs) for svs, _, _ in self.recurrent_svs.values()])} recurrent SVs')
        ...
        for sv_num, sv in enumerate(self.svs):
            sv_set = sv.info['VSET']
            self.place_recurrent_sv(sv_num, roi_indexes)
            self.call_place_sv(sv, (sv_num,), sv_set, roi_indexes)
            if time.time() - t_last_status > 10:
                logger.info(f'Placed {sv_num} of {len(self.svs)} SVs and {len(self.placed_recurrent_svs)} recurrent SVs in {time.time() - t_start_placing:.1f}s')
                t_last_status = time.time()
        # Places potential remaining recurrent SVs.
        self.place_recurrent_sv(len(self.svs), roi_indexes, place_all=True)
        logger.info(f'Placed {len(self.svs)} SVs and {len(self.placed_recurrent_svs)} recurrent SVs in {time.time() - t_start_placing:.1f}s.')
    ...
    def get_breakend(self, containing_region=None, avoid_chrom=None, blacklist_regions=None,
                            roi_length=0, total_length=0, enable_overlap=False, operation=None):
        ...
        while breakend is None and num_tries < max_random_tries:
            if operation:
                # Reset the operation overlap properties if an iteration failed
                operation.overlap_position = None
                operation.origin_length = None
                operation.overlap_op_id = None
            ...
            breakend, ref_roi = self.get_random_breakend(containing_region=containing_region, chromosomes=chromosomes,
                                                         ...
                                                         total_length=total_length, enable_overlap=enable_overlap,
                                                         operation=operation)
            num_tries += 1
        print('BREAKEND', breakend, 'REF_ROI', ref_roi)
        if breakend is None:
            # The breakend failed to be assigned by random sampling, looking for an available region fitting the SV.
    ...
    def get_random_breakend(self, containing_region=None, chromosomes=None, blacklist_regions=None,
        ...
        if not enable_overlap:
            ...
            ref_roi = self.get_reference_interval(region, self.reference_regions)
            if (len(ref_roi) != 1) or (breakend + roi_length > ref_roi[0].data.end):
                # If the reference interval is split or not big enough it fails
                return None, None
        else:
            # If enable_overlap, we look over all the regions of the chromosomes, without the deleted regions, plus the inserted regions
            available_regions = list(self.regions_for_overlap.chrom2itree[chrom]) + list(self.inserted_regions.chrom2itree[chrom])
            weights = [region.data.length() if region not in self.inserted_regions.chrom2itree[chrom] else self.inserted_lengths[region.data] for region in available_regions]
            total_weights = sum(weights)
            # Randomly select a region among all the available ones (weighted by the region length)
            region_idx = np.random.choice(len(available_regions), p=[weight / total_weights for weight in weights])
            ref_roi = available_regions[region_idx]
            # Randomly place the breakend inside the region
            breakend = ref_roi.data.start + np.random.randint(0, weights[region_idx])
            if ref_roi in self.inserted_regions.chrom2itree[chrom]:
                # Breakend in an inserted sequence
                # Length insertion marks as much of the SV is contained in the inserted region, to know how much more can be placed.
                region = Region(start=ref_roi.data.start, end=ref_roi.data.start, chrom=chrom,
                                length_insertion=min(weights[region_idx], total_length))
                operation.overlap_position = breakend
                operation.origin_length = total_length
                operation.overlap_op_id = ref_roi.data.op_id
                # Get a valid reference region (not reduced to a single point
                ref_roi = self.get_reference_interval(region, self.regions_for_overlap)
            else:
                # Breakend in an original sequence
                region = Region(start=breakend, end=breakend, chrom=chrom)
                ref_roi = [ref_roi]
        #(blacklist_regions.chrom2itree[chrom], breakend, blacklist_regions.strictly_contains_point(breakend, chrom))
        if (chrom in blacklist_regions.chrom2itree) and (
                blacklist_regions.strictly_contains_point(breakend, chrom)):
            return None, None
    ...
    def get_breakend_from_regions(self, containing_region=None, avoid_chrom=None, blacklist_regions=None,
        ...
        for chrom_tree, tree in chrom_trees.items():
            ...
            if containing_region is not None:
                ...
                for overlap in overlaps:
                    ...
                    # padding in case we only have a point left
        ...
        # Padding so the interval is not empty
    ...
    def get_overlap_region(self, sv_category, roi_index, init_roi, anchor_length=None,
                           overlap_mode=None, roi_filter=None, enable_overlap=False):
        # The region is constrained we use the interval tree defined from the bed file
        roi_list = self.rois_overlap[sv_category]
        if enable_overlap:
            # If overlap is allowed, regions can be selected independently of the presence of an SV, the original regions are used.
            roi_list = self.orig_rois_overlap[sv_category]

        # Add the beginning of the ROIs list at the end of the list of ROIs to check to ensure all are checked in case we run out.
        ordered_roi = (roi_list[roi_index:] + roi_list[:init_roi])
        for region in ordered_roi:
            roi_index = (roi_index + 1) % len(roi_list)
            if not roi_filter.satisfied_for(region): continue
            valid_region, ref_roi = self.check_interval_overlap(region, roi_filter, anchor_length, overlap_mode, enable_overlap)
            if valid_region is not None:
                return valid_region, ref_roi.data, roi_index
        return None, None, None

    def check_interval_overlap(self, region, roi_filter, anchor_length, overlap_mode, enable_overlap=False):
        if not enable_overlap:
            # Only keep regions of the interval that are in the reference
            ref_intervals = self.get_reference_interval(region, self.reference_regions)
        else:
            # If enable_overlap, the chromosome updated with the insertions and deletions
            ref_intervals = self.get_reference_interval(region, self.regions_for_overlap)
    ...
    def get_reference_interval(self, roi, reference):
        #Get the reference interval overlapping a ROI if any.
        for chrom_tree, tree in reference.chrom2itree.items():
    ...
    def propagate_placement(self, placement_dict, roi, ref_roi, breakends, lengths,
            ...
            for pos, breakend in enumerate(breakends):
                ...
                if distance is None:
                    ...
                    # Check if the SV overlaps an insertion target to adapt the position
                    if roi.chrom in self.inserted_regions.chrom2itree.keys():
                        # If the current SV is not recurrent, it can only overlap INS from recurrent SVs.
                        # This is ensured by the fact that the ref_roi cannot overlap insertion targets of non-recurrent SVs if the SV is not recurrent.
                        overlapping_insertions = list(self.inserted_regions.chrom2itree[roi.chrom].overlap(Interval(roi.start, position)))
                        # Sort the insertion points by position, in reverse order if backward
                        overlapping_insertions = sorted(overlapping_insertions, key=lambda i: i.begin, reverse=backward)
                        print('OVERLAP INSERTION', overlapping_insertions, 'FROM POS', position)
                        for insertion in overlapping_insertions:
                            # Check if the overlap is still happening
                            if not (position < insertion.data.start < roi.start or roi.start < insertion.data.start < position): continue
                            insertion_length = self.inserted_lengths[insertion.data]
                            # Compute the maximum size of the overlap
                            overlap_length = min(insertion_length, insertion.data.start - position if backward else
                                                                    position - insertion.data.start)
                            distance -= overlap_length
                            # Update the position
                            position = roi.start - distance if backward else roi.start + distance
                            print('UPDATED DISTANCE by', overlap_length, 'New Position', position)
    ...
    def place_sv(self, sv, roi_index):
        ...
        sv_set = sv.info['VSET']
        enable_overlap = sv_set in self.recurrent_svs
        init_roi = 0
        ...
        while not sv.is_placed() and (n_placement_attempts < max_tries):
            ...
            if sv.anchor is not None:
                ...
                if not sv.is_interchromosomal:
                    # total_length is the sum of lengths between dispersions for an interchromosomal SV (sum of the length per chr).
            ...
            if roi.length_insertion:
                # The SV is in an insertion region, we have to reduce its length by what has already been placed in the region.
                breakend_interval_lengths[0] -= roi.length_insertion
