...
class RegionSet:
    ...
    def overlaps_sv_placement(self, sv, genotype) -> bool:
        """
        ...
        for region in sv.get_regions():
            ...
            for hap in range(self.num_hap_tree):
                ...
                tree = self.chrom2itree[region.chrom][hap]

                if region.is_empty():
                    # Insertion points overlap only if they are included in a region
                    overlaps = tree.at(region.start)
                    if any(interval.data.start + 0.1 < region.start < interval.data.end - 0.1 for interval in overlaps):
                        return True
                else:
