...
from insilicosv.sv_defs import Syntax, VariantType
...
class RegionSet:
    ...
    def __contains__(self, region):
        overlap = []
        for hap in range(self.num_hap_tree):
            overlap += list(self.chrom2itree[region.chrom][hap].overlap(region.start, region.end))
    ...
    def overlaps_sv_placement(self, sv: 'SV', genotype) -> bool:
        """
        Checks if any of the regions generated by the SV overlap with any region in the RegionSet.
        """
        for region in sv_regions:
            if region.chrom not in self.chrom2itree:
                continue
            
            # Add some padding if the region is an insertion point
            start_pos = region.start - 0.1 if region.is_empty() else region.start
            end_pos = region.end + 0.1 if region.is_empty() else region.end

            # Check for overlaps across corresponding haplotype trees
            for hap in range(self.num_hap_tree):
                if hap != self.num_hap_tree - 1 and not genotype[hap]: continue
                if self.chrom2itree[region.chrom][hap].overlap(start_pos, end_pos):
                    return True
